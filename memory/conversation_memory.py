...
class ConversationMemory:
    ...
    Manages short-term conversation context, enforces sliding context window limits,
    and handles offline persistent memory syncing.
    """
    ...
    def _get_or_create_session(self, session_id: str) -> SessionData:
        if session_id not in self._active_sessions:
            ...
            if loaded:
                ...
                self._active_sessions[session_id] = SessionData(session_id=session_id)
    ...
    def add_assistant_message(self, content: str, session_id: Optional[str] = None) -> None:
        sid = session_id or self.default_session_id
        session = self._get_or_create_session(sid)
    ...
    def get_recent_messages(self, session_id: Optional[str] = None, limit: Optional[int] = None) -> List[Message]:
        sid = session_id or self.default_session_id
        session = self._get_or_create_session(sid)
        max_limit = limit or self.max_context_messages
        return session.messages[-max_limit:]

    def get_formatted_context(self, session_id: Optional[str] = None, limit: Optional[int] = None) -> str:
